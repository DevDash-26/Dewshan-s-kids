...
import { AlertOctagon, BookOpen, CalendarDays, ClipboardList, DoorOpen, LayoutDashboard, LogOut, Menu, MessageSquareText, PackageSearch, Search, Sparkles, Users, X, type LucideIcon } from 'lucide-react'
import { useAuth } from '../../context/AuthContext'
import logoUrl from '../../../images/UCL-Logo.png'
...
export function AppLayout() {
  ...
  const staffLinks: NavItem[] = [['/staff', 'Staff Console', ClipboardList, true]]
  ...
  return <div className="min-h-screen bg-[#f7f7f5] text-[#111214]">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[278px] -translate-x-full flex-col bg-[#111214] p-5 text-white shadow-2xl transition-transform duration-200 lg:translate-x-0 ${open ? 'translate-x-0' : ''}`}>
      <div className="mb-10 flex items-center gap-3"><img src={logoUrl} alt="UCL Connect" className="h-10 w-10 rounded-xl object-cover" /><div><div className="font-black tracking-tight">UCL CONNECT</div><div className="text-[10px] uppercase tracking-[.2em] text-white/40">Your campus. Connected.</div></div><button onClick={() => setOpen(false)} className="ml-auto rounded-lg p-2 text-white/60 hover:bg-white/10 lg:hidden" aria-label="Close menu"><X size={18} /></button></div>
